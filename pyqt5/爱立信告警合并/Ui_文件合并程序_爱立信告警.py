...
class Ui_MainWindow(QWidget):
    ...
    def merge_csv(self,fname):
        ...
        if isinstance(fname,list):
            for file in fname:
                df_tmp = read_csv(file, engine = 'python',skiprows = self.skip_rows, sep = '|')
                df_merge = df_merge.append(df_tmp)
        else :
            df_merge = read_csv(fname, engine = 'python',skiprows = self.skip_rows, sep = '|')
        file_path = self.get_file_path(fname)
        with open(file_path + '合并后文件'+ '.csv','w',newline = '') as writer:
