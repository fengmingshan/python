...
def show_rrc_recon_list():
    ...
    top_cells = session_rrc.execute(
        "SELECT 站号, `小区号`, `小区名称`, `RRC重建请求数目`, a.rrc_recon_ratio, a.rrc_recon_count/b.rrc_recon_sum as '占全网比例' FROM (SELECT 站号, `小区号`, `小区名称`, `周`, `RRC重建请求数目`, `RRC重建立比例` as 'rrc_recon_ratio', `RRC重建请求数目` as 'rrc_recon_count' from `rrc重建`) as a, (SELECT sum(`RRC重建请求数目`) as 'rrc_recon_sum' from `rrc重建`) as b WHERE a.`周` = {cur_week} ORDER BY a.rrc_recon_count/b.rrc_recon_sum DESC LIMIT 20".format(
            cur_week=cur_week))
...
def show_rrc_recon_detail(cell_info):
    ...
    HO_re_in_chart = draw_bar_stack_5(['' for x in range(len(ho_reason_ne))], '资源分配失败', ho_re_in_1, '源侧取消切换',
                                      ...
                                      ho_re_in_5, '切换入准备失败', )
    hcell = "'" + str(enb).split('.')[0] + '_' + str(cell) + "'"
