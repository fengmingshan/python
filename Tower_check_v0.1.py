...
def main():  
    ...
    #将上月账单和本月账单按右合并，合并成一张表 df_merge
    df_merge=pd.merge(df_old,df_new,how='right',on='产品业务确认单编号',suffixes=('上月','本月'))  #对上月账单和本月账单进行右合并，关键字是站址编码，列标后面添加标记‘左表和右表’
    ...
    df_add=df_merge[df_merge['账期月份上月']=='Null']     #对右合并之后的表格筛选站址编码='Null'的行,
