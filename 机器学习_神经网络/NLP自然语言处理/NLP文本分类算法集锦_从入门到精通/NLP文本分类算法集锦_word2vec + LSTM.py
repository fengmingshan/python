...
# 在模型拟合时，使用early stopping这个回调函数（Callback Function）来停止在最佳的迭代节点

earlystop = EarlyStopping(monitor='val_loss', min_delta=0, patience=3, verbose=0, mode='auto')
model.fit(x_train_pad, y=y_train_enc, batch_size=512, epochs=10,
          verbose=1, validation_data=(x_test_pad, y_test_enc), callbacks=[earlystop])


# 基于前面训练的Word2vec词向量，使用双向LSTM模型
model = Sequential()
model.add(Embedding(len(word_index) + 1,
                    100,
                    weights=[embedding_matrix],
