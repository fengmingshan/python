...
# 按照用户特征的大类来进行缺失值处理：先处理 df_customer 信息
...
df_customer.cust_access_net_dt = df_customer.cust_access_net_dt.map(lambda x:(parse(today)-parse(x)).days)
...
#with open('df_customer.csv','w') as writer:
#    df_customer.to_csv(writer,index =False)
