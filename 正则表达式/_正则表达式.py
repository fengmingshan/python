...
string = '''
第1题
问题： gNodeB 通过 PDCCH的DCI
格式 UO/Ul 调整 TPC取值， DCI长度
是多少？
A. lbit
B. 4bit
C. 2bit
...
pattern_question  = re.compile(p_question)
# 使用.group(1)提取括号中的内容,就得到了字符 A.之前的所有内容
question = [re.search(pattern_question,x).group(1) for x in questions]
