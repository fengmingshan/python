...
df_ho = pd.read_csv('./2020_06_01_14_42_05_491_qj_wxzx_wangxin_26575.csv',header=0,names= title, encoding = 'gbk')
...
df_ho['周'] = df_ho['开始时间'].map(lambda x:int(pd.to_datetime(x).strftime("%V")))
df_ho['key'] = df_ho['周'].map(str) + '_' + df_ho['网元'].map(str) + '_' + df_ho['小区'].map(str)
...
# 数据入库
# =============================================================================
for i in range(ceil(len(df_ho)/50000)):
    start = i*50000
    begin = (i+1)*50000 if i<17 else  len(df_ho)
    ...
    handover_data = [Nei_handover(
        ...
        df_ho.loc[start:end,'key'],
        df_ho.loc[start:end,'开始时间'],
        df_ho.loc[start:end,'结束时间'],
        df_ho.loc[start:end,'周'],
        df_ho.loc[start:end,'子网'],
        df_ho.loc[start:end,'子网名称'],
        df_ho.loc[start:end,'网元'],
        df_ho.loc[start:end,'网元名称'],
        df_ho.loc[start:end,'小区'],
        df_ho.loc[start:end,'小区名称'],
        df_ho.loc[start:end,'eNodeB'],
        df_ho.loc[start:end,'eNodeB名称'],
        df_ho.loc[start:end,'邻区'],
        df_ho.loc[start:end,'邻区关系'],
        df_ho.loc[start:end,'产品'],
        df_ho.loc[start:end,'切换出请求总次数'],
        df_ho.loc[start:end,'切换入请求总次数'],
        df_ho.loc[start:end,'切换出成功次数'],
        df_ho.loc[start:end,'切换出失败次数'],
        df_ho.loc[start:end,'切换入成功次数'],
        df_ho.loc[start:end,'切换入失败次数'],
        df_ho.loc[start:end,'邻区漏配'],
        df_ho.loc[start:end,'切换出成功率'],
        df_ho.loc[start:end,'切换出执行失败次数'],
        df_ho.loc[start:end,'切换出准备失败次数'],
        df_ho.loc[start:end,'切换入成功率'],
        df_ho.loc[start:end,'切换入执行失败次数'],
        df_ho.loc[start:end,'切换入准备失败次数'],
        df_ho.loc[start:end,'切换出执行失败次数_源侧发生重建立'],
        df_ho.loc[start:end,'切换出执行失败次数_等待UECONTEXTRELEASE消息超时'],
        df_ho.loc[start:end,'切换出执行失败次数_其它原因'],
        df_ho.loc[start:end,'切换入执行失败次数_RRC重配完成超时'],
        df_ho.loc[start:end,'切换入执行失败次数_源侧取消切换'],
        df_ho.loc[start:end,'切换入执行失败次数_其他原因'],
        df_ho.loc[start:end,'切换入执行失败次数_目标侧发生重建立'],
        df_ho.loc[start:end,'切换入准备成功次数'],
        df_ho.loc[start:end,'切换入准备失败次数_资源分配失败'],
        df_ho.loc[start:end,'切换入准备失败次数_源侧取消切换'],
        df_ho.loc[start:end,'切换入准备失败次数_其它原因'],
        df_ho.loc[start:end,'切换入准备失败次数_目标侧发生重建立'],
        df_ho.loc[start:end,'切换入准备失败次数_传输资源受限'],
        df_ho.loc[start:end,'切换出准备成功次数'],
        df_ho.loc[start:end,'切换出准备失败次数_等待切换响应定时器超时'],
        df_ho.loc[start:end,'切换出准备失败次数_目标侧准备失败'],
        df_ho.loc[start:end,'切换出准备失败次数_其它原因'],
        df_ho.loc[start:end,'切换出准备失败次数_源侧发生重建立'],
        df_ho.loc[start:end,'切换出准备失败次数_用户未激活'],
        df_ho.loc[start:end,'切换出准备失败次数_传输资源受限']
    )]
