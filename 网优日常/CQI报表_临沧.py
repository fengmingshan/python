...
df_qujing = df_yunnan[(df_yunnan['区域'] == '临沧市')]
...
df_lincang_zte  = df_yunnan[(df_yunnan['区域'] == '临沧市')&(df_yunnan['厂家'] == '中兴')]
df_lincang_zte = df_lincang_zte.reset_index()    
df_lincang_zte['CQI优良比'] =  df_lincang_zte['CQI大于等于7次数']/df_lincang_zte['CQI上报总次数']

df_zte_L1800  = df_yunnan[(df_yunnan['区域'] == '临沧市')&(df_yunnan['厂家'] == '中兴')&(df_yunnan['是否800M设备'] == '否')]
df_zte_L1800 = pd.pivot_table(df_zte_L1800, index=['日期','区域'], 
                                           values =['CQI上报总次数','CQI大于等于7次数' ], 
                                           aggfunc = {'CQI上报总次数':np.sum,'CQI大于等于7次数':np.sum})     
df_zte_L1800 = df_zte_L1800.reset_index()    
df_zte_L1800['CQI优良比'] =  df_zte_L1800['CQI大于等于7次数']/df_zte_L1800['CQI上报总次数']


df_zte_L800  = df_yunnan[(df_yunnan['区域'] == '临沧市')&(df_yunnan['厂家'] == '中兴')&(df_yunnan['是否800M设备'] == '是')]
df_zte_L800 = pd.pivot_table(df_zte_L800, index=['日期','区域'], 
                                           values =['CQI上报总次数','CQI大于等于7次数' ], 
                                           aggfunc = {'CQI上报总次数':np.sum,'CQI大于等于7次数':np.sum})     
df_zte_L800 = df_zte_L800.reset_index()    
df_zte_L800['CQI优良比'] =  df_zte_L800['CQI大于等于7次数']/df_zte_L800['CQI上报总次数']

    
# =============================================================================
# 质优小区
# =============================================================================
df_zte_good = df_lincang_zte[df_lincang_zte['CQI优良比']>0.93]   
...
# 质差小区
# =============================================================================
df_zte_worse = df_lincang_zte[df_lincang_zte['CQI优良比']<0.93]  
...
# 画中兴l1800_CQI优良率
# =============================================================================
y1 = df_zte_L1800['CQI优良比'].map(lambda x:x*100).T.values
x1 = df_zte_L1800['日期'].T.values
plt.figure(figsize=(12, 4))
plt.xticks(range(len(x1)), x1,fontsize=8)
plt.plot(range(len(x1)),y1,label='CQI优良比',linewidth=3,color='r',marker='o',markerfacecolor='blue',markersize=6) 
for a,b in zip(range(len(x1)),y1):
    plt.text(a,b*1.001,  '%.2f%%' % b, ha='center', va= 'bottom',fontsize=9)
plt.xlabel('日期')
plt.ylabel('中兴L1800_CQI优良比')
plt.title('中兴L1800_CQI优良比变化情况')
plt.legend(loc='center right')
plt.savefig(pic_path + "中兴L1800_CQI优良比.png",format='png', dpi=400)  
plt.close()
...
# 画中兴L800_CQI优良率
# =============================================================================
y1 = df_zte_L800['CQI优良比'].map(lambda x:x*100).T.values
x1 = df_zte_L800['日期'].T.values
plt.figure(figsize=(12, 4))
plt.xticks(range(len(x1)), x1,fontsize=8)
plt.plot(range(len(x1)),y1,label='CQI优良比',linewidth=3,color='r',marker='o',markerfacecolor='blue',markersize=6) 
for a,b in zip(range(len(x1)),y1):
    plt.text(a,b*1.001,  '%.2f%%' % b, ha='center', va= 'bottom',fontsize=9)
plt.xlabel('日期')
plt.ylabel('中兴L800_CQI优良比')
plt.title('中兴L800_CQI优良比变化情况')
plt.legend(loc='center right')
plt.savefig(pic_path + "中兴L800_CQI优良比.png",format='png', dpi=400)  
plt.close()
...
with open(out_path + current_date + '_' + 'OMMB1_command_CQI周期.txt','a') as f:
    for i in range(0,len(df_good_ommb1),1):
...
with open(out_path + current_date + '_' +  'OMMB2_command_CQI周期.txt','a') as f:
    for i in range(0,len(df_good_ommb2),1):
        line = r'UPDATE:MOC="PhyChannel",MOI="{0}",ATTRIBUTES="cqiRptPeriod=\"0;1;2\"",EXTENDS="";'\
...
with  pd.ExcelWriter(out_path + '临沧CQI优良率' + current_date + '.xlsx')  as writer:  #输出到excel
    book = writer.book 
    sheet = book.add_worksheet('本月MR指标')
    sheet.insert_image('A2' , pic_path + "全省CQI优良比.png")
    sheet.insert_image('A23', pic_path + "全市CQI优良比.png")
    sheet.insert_image('A44', pic_path + "中兴L1800_CQI优良比.png")
    sheet.insert_image('A65', pic_path + "中兴L800_CQI优良比.png")
