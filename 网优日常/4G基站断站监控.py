...
# 通过cmd ping测试基站
# =============================================================================
def ping_test(df_bts,sleeptime):
    with open(data_path + '')
    for i in range(0,len(df_bts)):
        ip = df_bts.loc[i,IP']
        command_line = list('ping '+ x for x in ip_list)   # 生成ping命令列表
        res = os.popen('ping 127.0.0.0 -n 1').readlines()
...
def task():
    ...
    if len(return_list) > 0:
        ...
        if (t1-t0).seconds <1320:  # 计算程序运行时间，如果小于22分钟，则重测测试失败的部分 
...
# =============================================================================
    ...
    if len(file_list) > 0:    # 如果取到的文件列表不为空，则开始处理文件     
        ...
        for i in range(0,len(break_bts),1):
            ...
            df_break_tmp['持续时长(分)'] = df_break_tmp['持续时长(分)'].map(lambda x:round(x,0))
