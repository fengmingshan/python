...
df_tmp.drop_duplicates('手机号', keep='first', inplace=True)
...
df_tmp.rename(columns={'小区':'基站名称','终端品牌':'手机品牌','终端型号':'手机型号'},inplace =True)
...
for 区县 in 区县列表:
    ...
    with pd.ExcelWriter(out_path + 区县 + '_返乡数据.xlsx') as writer: #不用保存和退出，系统自动会完成
        df_区县.to_excel(writer, 区县 + '返乡数据',index = False,) 
