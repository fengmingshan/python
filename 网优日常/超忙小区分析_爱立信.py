...
def read_csv_block(file_name):
    file_content = pd.read_csv(file_name ,engine= 'python',names = title_list, chunksize = 10000)
...
df_eric['Total_throughput'] = round(((df_eric['Air Interface_Traffic_Volume_UL_MBytes']
    + df_eric['Air Interface_Traffic_Volume_DL_MBytes'])/1024),2)
...
df_high_Data_800 = df_eric_L800[(df_eric_L800['Total_throughput'] >= 1.5) & (df_eric_L800['DL_Util_of_PRB'] >= 0.5) ]
df_massive_users_800 = df_eric_L800[(df_eric_L800['Max number of UE in RRc'] >= 50) & (df_eric_L800['DL_Util_of_PRB'] >= 0.5) ]
...
for week in list(set(df_busy_cell_800['week'])):
     ...
     df_pivot_800 = pd.pivot_table(df_800, index=['DATE_ID','eNodeB','cell','EUTRANCELLFDD'],
                                            ...
                                            aggfunc = {'Total_throughput':np.max,
