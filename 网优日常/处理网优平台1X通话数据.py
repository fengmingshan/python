...
df_1X_call = df_1X_call[(df_1X_call['cell_name'].str.contains(
    'QJ')) & (~df_1X_call['cell_name'].str.contains('六螺蛳'))]
...
df_number_imsi2 = df_number_imsi2.sort_values(
    by=['数据生成时间', 'ms_num'], ascending=False)
...
df_abnormal = pd.read_csv(billing_path + abnormal_user,
...
df_valid_user = df_abnormal[~df_abnormal['状态'].isin(
    ['用户申请拆机(移动业务)', '用户要求停机', '双停', '挂失'])]
...
df_1X_call_pivot = pd.pivot_table(df_1X_call,
                                  index=['用户号码', 'imsi', '用户状态',
                                         '区县', '乡镇', '无线接入次数'],
                                  values=['通话次数'],
                                  aggfunc={'通话次数': np.sum})
df_1X_call_pivot.reset_index(inplace=True)
df_1X_call_pivot['常驻地占比'] = round(
    df_1X_call_pivot['通话次数']/df_1X_call_pivot['无线接入次数'], 4)
...
def calc_user_home(df):
    ...
    for number in user_number_set:
        ...
        df_uesr_home_all = Append(
            df_uesr_home_all, df_user_calls[df_user_calls['连接次数'] == max_call_number])
        if i % 100 == 0:
...
df_user_data_traffic = df_user_data_traffic[~df_user_data_traffic['用户号码'].map(
    lambda x:str(x)[:4] == '0874' or str(x)[:3] == '0874')]
df_user_data_traffic['用户号码'] = df_user_data_traffic['用户号码'].astype(np.int64)
df_user_data_traffic['日均流量_MB'] = round(
    df_user_data_traffic['上网流量']/(1024*1024*30), 2)
df_user_data_traffic['平均网速_KBps'] = round(
    df_user_data_traffic['上网流量']/df_user_data_traffic['上网时长']/1024, 0)

df_user_data_traffic['user_info'] = df_user_data_traffic.apply(
    lambda x: judge_uesr_type(x.通话次数, x.上网流量, x.日均流量_MB), axis=1)
df_user_data_traffic['user_type'] = df_user_data_traffic['user_info'].map(
    lambda x: x[0])
df_user_data_traffic['user_level'] = df_user_data_traffic['user_info'].map(
    lambda x: x[1])
...
df_uesr_home_all['通话时长'] = df_uesr_home_all['用户号码'].map(
    user_call_druation_dict)
