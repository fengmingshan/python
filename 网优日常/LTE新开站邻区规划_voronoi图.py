...
def plan_first_layer(plan_bts_name,df_cell):
    ...
    first_layer_regions = set(reduce(operator.concat, list(df_first_layer['regions'])))
    ...
    for cell in plan_cell:
        for i in range(0,len(df_cell)):
            if len(first_layer_regions & set(df_cell.loc[i,'regions'])) > 0 \
                ...
                df_cell.loc[i,'degree'] = clac_degree(S_Lon,S_Lat,df_cell.loc[i,'LONB'],df_cell.loc[i,'LATB'])
                df_cell['opposite'] = calc_azimuth_difference(df_cell.loc[i,'degree'],S_Azimuth,df_cell.loc[i,'Azimuth'])
            else :
                df_cell.loc[i,'second_layer'] = 0
            df_second_layer = df_cell[(df_cell['second_layer'] == 1)&(df_cell['opposite'] == 'Yes')]
