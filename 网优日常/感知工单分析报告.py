...
df_mr.rename(columns = {'|≥-105dBm采样点':'≥-105dBm','|≥-110dBm采样点':'≥-110dBm','|≥-115dBm采样点':'≥-115dBm','|≥-120dBm采样点':'≥-120dBm','|≥负无穷采样点':'≥负无穷'},inplace = True)
df_mr[['≥-105dBm',
       '≥-110dBm', '≥-115dBm', '≥-120dBm', '≥负无穷']].T.plot(kind='bar',figsize = (10,7), title='MR覆盖分布')
...
# 添加标题 等级0
document.add_heading(analyze_cell_name + '感知质差分析', level=0)

# 添加第一段
document.add_heading('一、小区覆盖分析', level=1)

# 添加第一段的两个小节

document.add_paragraph('MR覆盖情况', style='List Number')
# 添加MR覆盖率图片
document.add_picture(pic_path + 'MR覆盖情况' + ".png", width=Inches(6))
# 添加MR覆盖率描述
paragraph1_1 = document.add_paragraph(
    analyze_cell_name + 'MR覆盖率:{mr_rate},MR覆盖指标{mr_lever}。'.format(mr_rate = mr_good_rate,mr_lever = mr_lever ), style=style_1)

document.add_paragraph('CQI优良比', style='List Number')
# 添加CQI图片
document.add_picture(pic_path + 'CQI优良比' + ".png", width=Inches(6))
# 添加CQI描述
paragraph1_2 = document.add_paragraph(
    analyze_cell_name + '平均CQI覆盖率:{cqi_rate},CQI覆盖率指标{cqi_lever}'.format(cqi_rate = cqi_rate,cqi_lever = cqi_lever ), style=style_1)


paragraph1_1.style = document.styles['Normal']
paragraph1_2.style = document.styles['Normal']

# 添加第二段
document.add_heading('二、小区话务负荷分析', level=1)

document.add_paragraph('用户数分析', style='List Number')

# 添加用户数图片
document.add_picture(pic_path + '最大RRC连接用户数_1' + ".png", width=Inches(6))

paragraph2 = document.add_paragraph(
    '小区' + analyze_cell_name + '忙时在线用户数XXX个', style=style_1)
#
#
## 添加图片
