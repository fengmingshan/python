...
for i in range(0,int(len(vol_lines)/100) + 1,1):
    with open(data_path + 'B2CMD' + str(i+1) + '.txt','a',encoding ='utf-8' ) as f:
        if i <  int(len(vol_lines)/100) :
            for j in range(i*100,(i+1)*100,1):
                f.write(vol_lines[j]) 
                f.write(sctp_lines[j]) 
        else:
            for j in range(i*100,len(vol_lines),1):
                f.write(vol_lines[j]) 
            for k in range(i*100,len(sctp_lines),1):
