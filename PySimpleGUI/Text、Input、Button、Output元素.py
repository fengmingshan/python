...
# @Last Modified time: 2019-11-20 17:43:51
...
while True:
    ...
    if event in ('开始'):
        if os.path.isfile(values[0]) and os.path.isfile(values[1]) and values[2].isdigit():
            print('你选择的文件0是{}。'.format(values[0]))
            print('你选择的文件1{}。'.format(values[1]))
            print('参数2设置为{}。'.format(values[2]))
            print('开始运行'.center(60,'#'),'\n')
            for i in range(10):
                time.sleep(1)
                print('第{}次运行，还有{}次。'.format(i+1,9-i))
                window.refresh()
        elif not os.path.isfile(values[0]):
            print('文件0设置错误，必须为文件！')
        elif not os.path.isfile(values[1]):
            print('文件1设置错误，必须为文件！！')
