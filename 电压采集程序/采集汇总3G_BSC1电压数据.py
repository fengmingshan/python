...
def storage_task(): 
    ...
    print(current_time + '数据采集完成：本次入库 %d 个文件！'% len(copy_files))
    
    sche.enter(3600,1,storage_task)  # 调用sche实力的enter方法创建一个定时任务，3600秒之后执行，任务内容执行storage_task()函数
...
def collect_task():
    ...
    for i in range(0,len(vo_file_list),1):
        df_file = pd.read_table(dest_path + vo_file_list[i],sep='\t',header=0,index_col=None,engine='python')  
    ...
    sche.enter(86400,1,collect_task)  # 调用sche实力的enter方法创建一个定时任务，一天之后执行，任务内容执行collect_task函数
   
      
sche.enter(3,1,storage_task)  # 调用sche实力的enter方法创建一个定时任务，12秒之后执行，任务内容执行task()函数

sche.enter(3,1,collect_task)  # 调用sche实力的enter方法创建一个定时任务，12秒之后执行，任务内容执行task()函数
sche.run()

print('----汇总电压数据任务开始----') # 提示信息
