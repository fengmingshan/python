...
df_res=pd.DataFrame(columns=['区县','扇区退服时长','A/B类平均断站时长','C/D类平均断站时长','排名'])
df_data=pd.DataFrame()
files=os.listdir(data_path)    #返回指定目录下的所有文件和目录名
for filename in files:
    if filename!=r'cell_num.xls' and filename!=r'Eric_info.xls' :
...
for i in range(0,len(df_data),1):  
    ...
    if '麒麟' in df_data.loc[i,'告警对象名称']:
        ...
        df_data.loc[i,'区县']='富源'
df_sum=df_data.groupby(by='区县').sum()
df_sum=df_sum.drop('')
df_sum=df_sum.sort_values(by='退服时长(分钟)',ascending=True) # 根据退服时长升序排列
df_res['区县']=df_sum.index
df_tmp=df_data[(df_data['基站等级']=='A')|(df_data['基站等级']=='B')]
df_sum_AB=df_tmp.groupby(by='区县').sum()
df_tmp=df_data[(df_data['基站等级']=='C')|(df_data['基站等级']=='D')]
df_sum_CD=df_tmp.groupby(by='区县').sum()
df_sum_CD=df_sum_CD.drop('')
for j in range(1,len(df_res)+1,1):
    df_res.loc[j,'扇区退服时长']=df_sum.at[df_res.loc[j,'区县'],'退服时长(分钟)']
    df_res.loc[j,'C/D类平均断站时长']=df_sum_CD.at[df_res.loc[j,'区县'],'退服时长(分钟)']
