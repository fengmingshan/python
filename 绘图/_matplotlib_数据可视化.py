...
# 创建一个画两列条形图的函数，直接输入两列数据，X轴的标签，和图片名称就能输出图片
def draw_bar_graph(df,col1,col2,country,pic_name)
    y1 = df[col1][df['week'] == week1 ].T.values
    y2 = df[col2][df['week'] == week2 ].T.values
    plt.figure(figsize=(6, 4))
    x1= range(0,len(country_list))
    x2 = [i+0.35 for i in x1]
    plt.bar(x1,y1,color='g',width = 0.3,alpha=0.6,label= col2)
    plt.bar(x2,y2,color='b',width = 0.3,alpha=0.6,label= col2)
    ...
    plt.title(pic_name)
    plt.savefig('./pic' + pic_name +".png",format='png', dpi=200)  
    plt.close()
